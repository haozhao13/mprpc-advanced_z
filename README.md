...
- **线程池**：任务队列 + 预创建线程，处理业务逻辑解耦。线程池管理的是“CPU 调度资源”（解决频繁创建/销毁线程的系统内核开销）

### 2. RPC 调用层
- **MprpcChannel (Client)**：
    - **序列化**：将 Protobuf 消息封装为 `Size + Header + Body`。
    - **服务发现**：连接 Zookeeper 获取服务列表。
    - **负载均衡**：使用一致性哈希算法选择目标节点。
    - **连接池**：复用 TCP 连接发送数据。连接池管理的是“网络连接资源”（解决频繁三次握手/四次挥手的网络开销）
- **RpcProvider (Server)**：
    - **反序列化**：解析 Header 和参数。
...
*   **操作内容**：需要提前编写好该文件内的逻辑，核心是**循环开启指定次数的 RPC 请求**。
    *   *注：本代码中示例为发起 500 次请求，循环调用 `Login` 方法。*
...
## 3. 第二阶段：启动 RPC 服务集群 (31 Nodes)
这是模拟生产环境的关键步骤，通过脚本自动部署 NODE_COUNT 个服务节点（本代码中示例为 31 个）。
...
*   **执行命令**：
    ...
    ./start_cluster.sh
    ```
    **执行后任选一个log文件，显示 ZK 初始化成功：**
    ![ZK初始化流程图](./docs/ZK初始化.png)
*   **脚本行为解析**：
    1.  **配置生成**：脚本会在 `/bin/conf/` 目录下自动生成 **31 份 `.conf` 配置文件**。
...
*   **预期流量**：
    *   客户端将发起 **500 次 RPC 请求**，这些请求将被打散并路由到刚才启动的 **31 个 RPC 服务节点**上。

## 5. 第四阶段：结果分析与可视化
压测结束后，通过脚本分析日志，验证负载均衡的效果。
...
*   **操作流程**：
    ...
    3.  **可视化展示**：
        ![日志统计结果图](./docs/log统计结果.png)
        *   **图表解读**：X 轴是端口号，表示不同的服务器；Y 轴是每个服务器所提供 rpc 请求的次数。
        *   **验证标准**：31 个柱状图（代表31个节点）高度应大致均衡。
*   **预期结论**：
...
*   **同步附上后续所做“1500 个请求打向 31 个 rpc 服务器的最终 log统计结果”**：
    ![日志统计结果图](./docs/1500-30_log统计结果.png)

## 6. 附加：长连接池内【连接数量限额机制】
引入该机制后，将资源管理从无限制扩张转变为“预先调配”。
*   这防止了因文件描述符（FD）耗尽导致的进程崩溃，还通过控制长连接数量，规避了内存溢出和 CPU 频繁切换的风险，增加了系统稳定性。
*   该机制降低了系统开销，将连接数控制在固定范围内，减少了由于连接超时后“自动消亡”时内核态与用户态的上下文切换，确保了 I/O 多路复用（如 epoll）的高效运行。

在不对**连接池内最大连接数**做限额时，做“1500 个 rpc 请求打向 31 个 rpc 服务器”的实验，最终结果：
**响应成功 1500 个；总计耗时 2918 ms；QPS：514.051**
*   **同步附上“当长连接池内连接数量限额至 100 后，1500 个 rpc 请求打向 31 个 rpc 服务器”的最终服务端运行结果**：
    ![consumer 结果图](./docs/1500-30_测试长连接数量限制机制&消亡机制.png)
*   由 QPS 可以看出，增加**连接池内最大连接数**的限额后，rpc 调用速度明显降低。
---

## 总结
这套压测流程完美复现了 **500 至 1500 次 rpc 请求 -> 31 个节点** 的分布式场景，模拟了生产环境中高并发 RPC 流量冲击服务端的真实状况。
在**资源调度**层面：引入线程池来管理 CPU 调度资源，有效避免了频繁创建/销毁线程带来的系统内核开销；同时，结合连接池复用 TCP 连接，解决了频繁网络握手/挥手的性能瓶颈。
在**框架**层面：通过一致性哈希算法，mprpc 框架不仅实现了负载均衡，还保证了路由的稳定，这在需要本地缓存（服务端）的业务场景中至关重要。
