...
这是模拟生产环境的关键步骤，通过脚本自动部署 NODE_COUNT 个服务节点（本代码中示例为 30 个）。
...
*   **验证结果**：
    *   此时，能看到 **30 个临时节点（Ephemeral Nodes）** 成功注册到 Zookeeper 中。
...
*   **同步附上后续所做“1500 个请求打向 30 个 rpc 服务器的最终 log统计结果”**：
...
在不对**连接池内最大连接数**做限额时，做“1500 个 rpc 请求打向 30 个 rpc 服务器”的实验，最终结果：
**响应成功 1500 个；总计耗时 2918 ms；QPS：514.051**
*   **同步附上“当长连接池内连接数量限额至 100 后，1500 个 rpc 请求打向 30 个 rpc 服务器”的最终服务端运行结果**：
    ![consumer 结果图](./docs/1500-30_测试长连接数量限制机制&消亡机制.png)
*   由 QPS 可以看出，增加**连接池内最大连接数**的限额后，rpc 调用速度明显降低。
---

## 总结
这套压测流程完美复现了 **500 至 1500 次 rpc 请求 -> 30 个节点** 的分布式场景，模拟了生产环境中高并发 RPC 流量冲击服务端的真实状况。
